...
from preprocessing.read_data import tf_record_parser, rescale_image_and_annotation_by_factor, \
    ...
    random_flip_image_and_annotation_up_and_down, download_resnet_checkpoint_if_necessary
...
class params():
    def __init__(self):
        self.resnet_model = "resnet_v2_101"
        self.number_of_classes = 6
        self.crop_size = 512
        self.batch_size = 6

        #学习策略参数（poly策略）
        self.starting_learning_rate = 0.007
        self.momentum = 0.9
...
#tfrecord文件路径
DATASET_DIR = './dataset/RGBIRDSM_512_tfrecords'
TRAIN_FILE = 'train.tfrecords'
VALIDATION_FILE = 'validation_no_stride.tfrecords'
...
training_dataset = training_dataset.map(random_flip_image_and_annotation_up_and_down)
...
with tf.variable_scope("optimizer_vars"):
    ...
    poly_learning_rate = tf.train.polynomial_decay(args.starting_learning_rate,
                                                   global_step = global_step,
                                                   decay_steps = args.training_steps,
                                                   end_learning_rate=0,
                                                   power = args.power)
    optimizer = tf.train.MomentumOptimizer(learning_rate=poly_learning_rate, momentum=args.momentum)
...
variables_to_restore = slim.get_variables_to_restore(exclude=[args.resnet_model + "/logits", "optimizer_vars",
                                                              ...
                                                              args.resnet_model + "/conv1"])

OA, update_op = tf.metrics.accuracy(tf.argmax(valid_labels_batch_tf, axis=1), tf.argmax(valid_logits_batch_tf, axis=1))

tf.summary.scalar('miou', OA)
...
with tf.Session() as sess:
    ...
    while True:
        ...
        for i in range(validation_steps):
            ...
            validation_average_miou += sess.run(OA)
